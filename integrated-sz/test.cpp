...
TEST_CASE("arena-map") {
  ...
  out.open("a3.txt", std::ios_base::out);
  res = dij.run(s, 3);
  vis = Visualizer(mapData, mapper).to_strings(s, res);
  for (string i: vis) out << i << endl;
  out.close();
...
TEST_CASE("badcase") {
  ...
  for (int i=0; i<mapper.node_count(); i++) {
    ...
    if (cnth2 < cnth1) {
      out << "find id:" << i << ", cnth1: " << cnth1 << ", cnth2: " << cnth2 << endl;
      if (cnth1 - cnth2 > v) {
        v = cnth1 - cnth2;
        worst = i;
      }
  ...
  out << "worst id: " << worst << ", diff: " << v << endl;
  out.close();
}
