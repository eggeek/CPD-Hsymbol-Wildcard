...
plotOrder <- function(path) {
  d = loadData(path)
  d$rank = as.integer(d$rank * 0xFFFFFF / nrow(d))
  colors = int2rgb(d$rank)
  plot(d$lat, d$lon, col=colors, pch='.')
  points(d[1:100,]$lat, d[1:100,]$lon, col=colors[1:100])
}
